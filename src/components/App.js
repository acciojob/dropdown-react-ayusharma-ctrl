	...
	return (
		<div id="main">
			...
			<select name="city" id="city" value={city} onChange={e => setCity(e.target.value)}>
				{
					states[state].city.map((e, i) => {
						return <option value={i} key={i}> {e.name} </option>
					})
				}
			</select>

			<select name="landmark" id="landmark" value={landmark} onChange={e => setLandmark(e.target.value)}>
				{
					states[state].city[city].landmarks.map((e, i) => {
						return <option value={i} key={i}> {e.name} </option>
					})
				}
			</select>
